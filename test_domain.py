...
def parse_option():
    ...
    # model dataset
    parser.add_argument('--source', type=str, default='CricketX',
                        choices=['CricketX', 'UWaveGestureLibraryAll',
                                 'InsectWingbeatSound', 'DodgerLoopDay',
                                 'MFPT', 'XJTU'])
    parser.add_argument('--target', type=str, default='DodgerLoopDay',
                        choices=['CricketX', 'UWaveGestureLibraryAll',
                                 'InsectWingbeatSound', 'DodgerLoopDay',
                                 'MFPT', 'XJTU'],
                        help='dataset')
    ...
    parser.add_argument('--model_name', type=str, default='SelfTime',
                        choices=['SupCE', 'InterSample', 'IntraTemporal', 'SelfTime'], help='choose method')
...
if __name__ == "__main__":
    ...
    config_dict = get_config_from_json('{}/{}_config.json'.format(
        opt.config_dir, opt.target))

    opt.class_type = config_dict['class_type']
    opt.piece_size = config_dict['piece_size']

    if opt.model_name=='InterSample' or opt.model_name=='SupCE' :
        model_paras = 'none'
    else:
    ...
    if backname=='init':
        log_dir = './results/{}/{}/{}/Random/{}/{}'.format(exp, opt.target, opt.source, '_'.join(opt.aug_type),
                                                    model_paras)
    else:
        log_dir = './results/{}/{}/{}/{}/{}/{}'.format(exp, opt.target, opt.source, opt.model_name, '_'.join(opt.aug_type),
                                                    model_paras)
    ...
    for seed in Seeds:
        ...
        for run in Runs:
            ...
            opt.ckpt_dir = './ckpt/{}/{}/{}/{}/{}/{}'.format(
                exp_ckpt, opt.model_name, opt.source, '_'.join(opt.aug_type),
            ...
            print('[INFO] Running at:', opt.target)

            x_train, y_train, x_val, y_val, x_test, y_test, nb_class, _ \
                = load_ucr2018(opt.ucr_path, opt.target)
            #########
            ## Test
            #########
            ...
            print("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format(
                opt.target, x_train.shape[0], x_test.shape[0], x_train.shape[1], nb_class, seed,
    ...
    ACCs[opt.target] = ACCs_seed_mean

    print("{}\t{}\t{}\t{}".format(
        opt.target, ACCs_seed_mean, ACCs_seed_std, MAX_EPOCHs_seed_max),
        file=file2print_test)
    file2print_test.flush()
